...
inline void Star::neg_velocity_x() {
    velocity.x = -velocity.x;
}

inline void Star::neg_velocity_y() {
    velocity.y = -velocity.y;
}

inline void  Star::move(const double time) {
...
inline double Star::get_center_x() const {
    return center.x;
}

inline double Star::get_center_y() const {
    return center.y;
}

inline COLOR Star::get_color() const {
    return color;
}
...
TPoint<T> *Star::get_tpoints() const {
    ...
    for (int i = 0; i < 10; i += 2) {
        points[i] = TPoint<T> {
            static_cast<T>(ldist * std::cos(angle + RAD(72.0) * i) + center.x),
...
Star::dPoint *Star::get_tpoints() const {
    ...
    for (int i = 0; i < 10; i += 2) {
        points[i] = dPoint {
            ldist * std::cos(angle + RAD(72.0) * i) + center.x,
            ldist * std::sin(angle + RAD(72.0) * i) + center.y
        };

        points[i + 1] = dPoint {
            ldist * ratio * std::cos(angle + RAD(72.0) * i + RAD(36.0)) + center.x,
...
Star::Point *Star::get_points() const {
...
void delete_star(Star *p_star) {
    delete p_star;
}

void star_neg_vx(Star *p_star) {
    p_star->neg_velocity_x();
}

void star_neg_vy(Star *p_star) {
    p_star->neg_velocity_y();
}

void star_move_for_time(Star *p_star, const double time) {
    p_star->move(time);
}

double star_get_center_x(const Star *const p_star) {
    return p_star->get_center_x();
}

double star_get_center_y(const Star *const p_star) {
    return p_star->get_center_y();
}

COLOR star_get_color(const Star *const p_star) {
    return p_star->get_color();
}

Star::Point *star_get_points(const Star *const p_star) {
    return p_star->get_points();
}
