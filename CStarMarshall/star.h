...
public:
    ...
    template<typename T> TPoint<T> *get_tpoints() const;
    template<>           dPoint    *get_tpoints() const;
...
extern "C" {
    __declspec(dllexport) Star *make_star(
        double x,         double y,
        double vx,        double vy,
        double angle,     double rotate,
        double ldistance, double ratio = 0.36
    ...
    __declspec(dllexport) Star *make_colored_star(
        COLOR  color,
        double x,         double y,
        double vx,        double vy,
        double angle,     double rotate,
